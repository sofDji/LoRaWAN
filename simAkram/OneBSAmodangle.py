...
try:
    sqliteConnection = sqlite3.connect('Nodesweightangle.db')
...
class myNode():
    def __init__(self, nodeid, bs, period, packetlen):
        ...
        self.angle = math.atan2(self.y - bsy, self.x - bsx)
        minangle = self.angle - (math.pi/12)
        maxangle = self.angle + (math.pi/12)

        if ((minangle < (-math.pi)) or (maxangle > math.pi)):
            if (minangle < - math.pi) :
                minangle += 2*math.pi
            elif (maxangle > math.pi) :
                maxangle -= 2*math.pi
            cursor.execute("Select ABS(distance - ?) AS closest, weight FROM NodesAngle WHERE angle > ? OR angle < ? ORDER BY closest LIMIT 1",(self.dist,minangle,maxangle))
        else:
            cursor.execute("Select ABS(distance - ?) AS closest, weight FROM NodesAngle WHERE angle > ? AND angle < ? ORDER BY closest LIMIT 1",(self.dist,minangle,maxangle))
...
def transmit(env,node):
    ...
    taille = len(node.SFs)
    max = 0
    for i in range(1,taille):                   # get best SF before any collisions
        if node.SFs[max] <= node.SFs[i]:
           max = i
    node.packet.sf = max + 7
    while True:
...
for n in nodes:
    ...
    cursor.execute("Insert INTO NodesAngle Values (?,?,?)",(n.dist,n.angle,str_of_sfs))
