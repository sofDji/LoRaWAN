...
def checkcollision(packet):
    sfCollisions = open("CollisionsOneBSFichier.txt", "w")
    now = datetime.now()
...
if len(sys.argv) >= 1:
    ...
    print "usage: ./OneBSFichier.py <avgsend>  <simtime> [collision]"
...
for ligne in fichier:
   if j == 0:
      ...
      x = ligne.split()
      node = myNode(j,bsId, avgSendTime,int(x[2]),float(x[0]),float(x[1]))
      nodes.append(node)
...
statistiques = open("statistiquesOneBSFichier.txt", "w")
now = datetime.now()
date_time = now.strftime("%m/%d/%Y, %H:%M:%S")
